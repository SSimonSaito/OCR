...
if (file_upload !=None):
    ...
    lang = set_language_list[set_language]

    # OCRエンジンを取得
    engines = pytesseract.get_languages(config='')  # pyocrが認識できるOCR外部ツールを検知
    engine = engines[0] # ツールを複数選択して、エラーにならないように１つだけ選択

    # 画像の文字を読み込む
    # engine.image_to_string(開いた画像,画像認識する言語)で画像分析開始し、分析結果をtxtに代入
    txt = pytesseract.image_to_string(Image.open(file_upload), lang=lang)
    st.write("OCR結果")
